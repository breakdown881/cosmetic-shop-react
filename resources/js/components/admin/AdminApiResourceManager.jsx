...
const formatCell = (row, column) => {
    ...
    if (column.type === 'boolean') {
        return value ? 'Ho?t d?ng' : 'Không ho?t d?ng';
    }
...
export default function AdminApiResourceManager({
    ...
    const loadRows = async () => {
        ...
        try {
            ...
            setErrorMessage(error.response?.data?.message ?? 'Không t?i du?c d? li?u.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setSuccessMessage(response.message ?? 'Luu thành công.');
            closeForm();
            await loadRows();
        } catch (error) {
            const errors = error.response?.data?.errors;
            const firstError = errors ? Object.values(errors).flat()[0] : null;
            setErrorMessage(firstError ?? error.response?.data?.message ?? 'Không luu du?c d? li?u.');
        }
    };

    const handleDelete = async (row) => {
        if (!window.confirm(labels.deleteConfirm ?? 'B?n có ch?c mu?n xóa?')) {
            return;
        }
        ...
        try {
            ...
            setErrorMessage(error.response?.data?.message ?? 'Không xóa du?c d? li?u.');
        }
    };
        ...
        <div className="container-fluid">
            ...
            {isFormOpen && (
                <form className="card card-body mb-3" onSubmit={handleSubmit}>
                    <h2 className="h5 mb-3">{editingRow ? labels.edit ?? 'S?a' : labels.add ?? 'Thêm'}</h2>
                    {fields.map((field) => (
            ...
            <div className="card mb-3">
                <div className="card-body">
                        ...
                        <table className="table table-hover" width="100%" cellSpacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th className="text-center" width="120">{labels.management ?? 'Qu?n lý'}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {rows.map((row) => (
                    ...
                    {isLoading && <p>Ðang t?i...</p>}
                    {!isLoading && rows.length === 0 && <p className="react-empty-state">{labels.empty ?? 'Không có d? li?u.'}</p>}
                </div>
            </div>
        </div>
    );
